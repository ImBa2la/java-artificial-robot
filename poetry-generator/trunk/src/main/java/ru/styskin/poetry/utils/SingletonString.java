...
public class SingletonString implements Comparable<SingletonString> {
    ...
    public static final String CONSONANT = "יצךםדרשחץפגןנכהזבעלסק";
    public static final String VOCALIC = "או¸טמףû‎‏ÿ";
    ...
    static {
    	for(int i=0; i < CONSONANT.length(); i++) {
    		CONSONANT_SET |= 1 << CONSONANT.charAt(i) - 'א';
    	}
    	for(int i=0; i < VOCALIC.length(); i++) {
    		VOCALIC_SET |= 1 << VOCALIC.charAt(i) - 'א';
    	}
    }
    ...
    private static int calculateSize(String s) {
    	...
    	for(int i=0; i < s.length(); i++) {
    		if((VOCALIC_SET & (1 << (s.charAt(i)-'א'))) > 0) {
    			res ++;    			
    		}
    	}
		if(s.length() > 2 &&
			(VOCALIC_SET & (1 << (s.charAt(s.length()-1)-'א'))) > 0 && 
			(VOCALIC_SET & (1 << (s.charAt(s.length()-2)-'א'))) > 0) {
			res --;
		}
    	return res;
    }
