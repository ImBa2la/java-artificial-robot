...
public class StringUtilsTest extends TestCase {
    ...
    public void testNormalize() throws Exception {
        ...
        assertEquals("пpeвeд и пaka!", StringUtils.normalize(string));

        string = "Превед  13и234   23пака!";
        assertEquals("пpeвeд 13 и 234 23 пaka!", StringUtils.normalize(string));
    ...
    public void testNormalizeRus2En() throws Exception {
        ...
        assertEquals("ceopakyxbtm", StringUtils.normalizeRus2En(toEnglish));

        String russian = "йцнгшщзъфывплджэячиью";
        assertEquals(russian, StringUtils.normalizeRus2En(russian));
    }

    public void testNormalizeToNumeric() throws Exception {
        String numbers = "32x43 x34 34x 23xxx";
        assertEquals("32 x 43 x 34 34 x 23 xxx", StringUtils.normalizeToNumeric(numbers));

        numbers = "3 x";
        assertEquals("3 x", StringUtils.normalizeToNumeric(numbers));

        numbers = "3";
        assertEquals("3", StringUtils.normalizeToNumeric(numbers));
    }
    ...
    public void testNotebookNormalize() throws Exception {
        ...
        assertEquals("пpeвeд и пaka!", StringUtils.notebookNormalize(string));

        string = "Превед  13и234,23пака!";
        assertEquals("пpeвeд 13 и 234.23 пaka!", StringUtils.notebookNormalize(string));

        assertEquals("", StringUtils.notebookNormalize(null));
    }

    public void testComplexNotebooksNormalize() {
        String result = StringUtils.notebookNormalize("Latitude 110L i910GML CM-360 1.4,512,60,15");
    ...
    public void testRemoveDotsAfterWords() throws Exception {
        assertEquals("a ", StringUtils.removeDotsAfterWords("a."));
    }
