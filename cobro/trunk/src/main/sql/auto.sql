...
select IF(currency_type = 'USD', price * 24.5, price) as price,  
case when mark in ('AUDI', 'BMW', 'MERCEDES', 'OPEL', 'PEUGEOT', 'PORSCHE', 'RENAULT', 'VOLKSWAGEN', 'VOLVO') then 'europe' 
when mark in ('VAZ', 'GAZ') then 'russia'
when mark in ('CHEVROLET', 'CHRYSLER', 'DODGE', 'FORD') then 'usa'
when mark in ('HONDA', 'LEXUS', 'MAZDA', 'MITSUBISHI', 'NISSAN', 'SUBARU', 'TOYOTA') then 'japan'
when mark in ('DAEWOO', 'KIA', 'HYUNDAI') then 'korea'
end origin,
